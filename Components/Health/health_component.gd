...
func _set_stat(value: CharacStatistics) -> void:
	if stat: stat.variable_stat_changed.disconnect(_on_stat_variable_stat_changed)
	if not value: return
	
	stat = value
	stat.variable_stat_changed.connect(_on_stat_variable_stat_changed)
	stat.set_variable_stat(HEALTH_STAT_NAME, stat.max_health)


#### LOGICS ####
func _hurt(damage: float, hurt_box: HurtBox, hit_box: HitBox) -> void:
	var dam: float= _compute_damage(damage)
	stat.set_variable_stat(HEALTH_STAT_NAME, health - dam)
...
func _on_stat_variable_stat_changed(stat_name: String, value: float) -> void:
	if stat_name != HEALTH_STAT_NAME: return
